  ...
  return withSecurity(request, async (req) => {
    ...
    if (!rateLimitResult.allowed) {
      securityLogger.log({
        ...
        details: { endpoint: '/api/payment/portone', action: 'POST' },
      });
    ...
    try {
      ...
      if (!session?.user?.id) {
        securityLogger.log({
          ...
          ip: context.ip,
          userAgent: context.userAgent,
          details: { endpoint: '/api/payment/portone', reason: 'Unauthenticated payment attempt' },
        });
        return NextResponse.json({ error: "로그인이 필요합니다" }, { status: 401 });
      }

      const { paymentId, productId } = await req.json();
      ...
      if (payment.status !== "PAID") {
        securityLogger.log({
          ...
          details: {
            endpoint: '/api/payment/portone',
            reason: 'Payment status not PAID',
      ...
      if (payment.amount.total !== productPrice) {
        securityLogger.log({
          ...
          details: {
            ...
            actualPrice: payment.amount.total,
            paymentId,
            productId,
          },
        });
      ...
      // 결제 성공 로깅
      securityLogger.log({
        type: 'LOGIN_SUCCESS', // 결제 성공 이벤트 (LOGIN_SUCCESS를 재활용)
        severity: 'low',
        ip: context.ip,
        userAgent: context.userAgent,
        userId: session.user.id,
